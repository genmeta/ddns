    ...
    fn endpoints<'a>(
        ...
        let endpoints = match selector {
            AddressSelector::WideArea => self
                .wide_area
                .get_or_init(|| stun_endpoints_from_iface(&self.iface)),
...
#[cfg(feature = "dquic-network")]
// Wide-area records require an address observed outside the local binding.
fn stun_endpoints_from_iface(iface: &BindInterface) -> Vec<EndpointAddr> {
    iface.with_components(|components, current| {
        ...
        tracing::trace!(
            ...
            endpoints = ?endpoints,
            "collected STUN-derived wide-area endpoints from interface"
        );
